...
public class HBaseClient extends com.yahoo.ycsb.DB
{
    ...
    //pradap: change start
    /* 
     Get the authorization configuration for a user !!.
     Hardcode the user to "user1"
    */ 
    private static final HAuthorization objAuthConfig = new HAuthorization("User1");
    //pradap: change end
    ...
    public void init() throws DBException
    {
      ...
      // pradap change start
      /*
       *  Set the port number and ip address here.
       *  Get it from the property and then update the ip address and port number
       */
      String _ipAddress = getProperties().getProperty("ipaddress");
      String _portNumber = getProperties().getProperty("portnumber");
      System.out.println("IpAddress : " + _ipAddress + " Portnumber : " + _portNumber);
      
      GlueZMQ.setIPAddres(_ipAddress);
	  GlueZMQ.setPortNumber(_portNumber);
    ...
    public void getHTable(String table) throws IOException
    {
        synchronized (tableLock) {
            // pradap change start
        	/*
        	 * Modify the contructor invocation: make it to pass through HTableAuth Wrapper
        	 * 
        	 */
        	/*
        	_hTable = new HTable(config, table);
            */
        	_hTable = new HTableAuth(objAuthConfig, config, table);
        	// pradap change end
